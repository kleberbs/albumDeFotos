...
public class Add extends HttpServlet {
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		User user = us.getCurrentUser();
		 if(user == null){		
			 ...
			 if(us.isUserAdmin() || accesResource(user.getEmail(),"/addRole")){
				 RequestDispatcher rd=req.getRequestDispatcher("/WEB-INF/Role/add.jsp");
				try {
					rd.forward(req, resp);
				} catch (ServletException e) {
					// TODO Auto-generated catch block
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {		
		...
		final Role role=new Role(req.getParameter("name"),Boolean.parseBoolean(req.getParameter("status")));
		pm.makePersistent(role);
		resp.getWriter().println("datos guardados");
		pm.close();
		resp.sendRedirect("/listRole");
	}
