		...
		protected void doGet(HttpServletRequest req, HttpServletResponse resp)
			...
			User user = us.getCurrentUser();
			 if(user == null){		
				 ...
				 if(us.isUserAdmin() || accesResource(user.getEmail(),"/listReservation")){
					 PersistenceManager pm = PMF.get().getPersistenceManager();
						Query q = pm.newQuery(Reservation.class);
						
						@SuppressWarnings("unchecked")
						List<Reservation> reservations = (List<Reservation>)q.execute("select from Reservation");
						
						req.setAttribute("reservations", reservations);
						try {
							req.getRequestDispatcher("/WEB-INF/Reservation/list.jsp").forward(req, resp);
							q.closeAll();
