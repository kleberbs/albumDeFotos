...
public class Edit extends HttpServlet {
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		User user = us.getCurrentUser();
		 if(user == null){		
			 ...
			 if(us.isUserAdmin() || accesResource(user.getEmail(),"/editReservation")){
				 final PersistenceManager pm = PMF.get().getPersistenceManager();
					String id = req.getParameter("id");
					Long idLong = Long.parseLong(id);
					try{
						Reservation reservation=pm.getObjectById(Reservation.class,idLong);
						req.setAttribute("reservation", reservation);
						RequestDispatcher rd = getServletContext().getRequestDispatcher("/WEB-INF/Reservation/edit.jsp");
						rd.forward(req, resp);
					}catch(Exception e){
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try{
			Reservation reservation=pm.getObjectById(Reservation.class,idLong);
			reservation.setEmail(req.getParameter("email"));
			reservation.setUser(req.getParameter("user"));
			reservation.setPlace(req.getParameter("place"));
			reservation.setDate(req.getParameter("date"));
			req.setAttribute("reservation", reservation);
			RequestDispatcher rd = getServletContext().getRequestDispatcher("/WEB-INF/Reservation/view.jsp");
			rd.forward(req, resp);
		}catch(Exception e){
				System.out.println(e);
